...
data). G8 Education Limited is the first Australia-jurisdiction company in this actor
family.
...
For G8 Education, `website` (`www.g8education.edu.au`) and the real archived
`source-url` (`g8education.edu.au`, no `www.` prefix) both reduce to the same naive
base-domain `edu.au` under `source-domain-mismatch-violations`' last-two-label
heuristic — the `.edu.au` two-part-TLD variant of the already-documented
`.co.jp`/`.co.uk` limitation noted in `tosmonitor.registry`'s own docstring, not a new
